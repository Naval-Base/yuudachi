...
export default class implements Command {
	...
	public async execute(message: APIMessage | APIInteraction, args: Args, locale: string): Promise<void> {
		...
		try {
			...
			void send(message, {
				content: i18next.t('command.mod.unban.success', { lng: locale, member: memberMention }),
			});
		} catch (e) {
			...
			throw new Error(i18next.t('command.mod.unban.errors.failure', { lng: locale, member: memberMention }));
		}
	}
}
