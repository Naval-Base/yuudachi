...
export async function react(
	...
	const [roles] = await sql<[{ reaction_role_id: `${bigint}` | '' | null }?]>`
	...
	if (action && !action.action_processed) {
		throw new Error(i18next.t('command.mod.restrict.react.errors.already_restricted', { lng: locale }));
	}
	...
	try {
		...
		void send(message, {
			content: i18next.t('command.mod.restrict.react.success', { member: memberMention, lng: locale }),
		});
	} catch (e) {
		...
		throw new Error(i18next.t('command.mod.restrict.react.errors.failure', { member: memberMention, lng: locale }));
	}
}
