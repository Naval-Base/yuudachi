...
export async function embed(
	...
	const [roles] = await sql<[{ embed_role_id: `${bigint}` | '' | null }?]>`
	...
	try {
		...
		void send(message, {
			content: i18next.t('command.mod.restrict.embed.success', { member: memberMention, lng: locale }),
		});
	} catch (e) {
		...
		throw new Error(i18next.t('command.mod.restrict.embed.errors.failure', { member: memberMention, lng: locale }));
	}
}
