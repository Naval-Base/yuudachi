...
export async function emoji(
	...
	if (!roles.emoji_role_id) {
		throw new Error(i18next.t('command.mod.restrict.emoji.errors.no_role'));
	}
	...
	try {
		...
		void send(message, {
			content: i18next.t('command.mod.restrict.emoji.success', { lng: locale, member: memberMention }),
		});
	} catch (e) {
		...
		throw new Error(i18next.t('command.mod.restrict.emoji.errors.failure', { lng: locale, member: memberMention }));
	}
}
