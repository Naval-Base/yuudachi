...
export async function mute(
	...
	const [roles] = await sql<{ mute_role_id: `${bigint}` | '' }>`
		select mute_role_id
		from guild_settings
	...
	if (!roles.mute_role_id) {
		throw new Error(i18next.t('command.mod.restrict.mute.errors.no_role'));
	}
	...
	if (action && !action.action_processed) {
		throw new Error(i18next.t('command.mod.restrict.mute.errors.already_restricted', { lng: locale }));
	}
	...
	try {
		...
		void send(message, {
			content: i18next.t('command.mod.restrict.mute.success', { lng: locale, member: memberMention }),
		});
	} catch (e) {
		...
		throw new Error(i18next.t('command.mod.restrict.mute.errors.failure', { lng: locale, member: memberMention }));
	}
}
