...
export async function tag(
	...
	const [roles] = await sql<{ tag_role_id: `${bigint}` | '' }>`
		select tag_role_id
		from guild_settings
	...
	try {
		...
		void send(message, {
			content: i18next.t('command.mod.restrict.tag.success', { lng: locale, member: memberMention }),
		});
	} catch (e) {
		...
		throw new Error(i18next.t('command.mod.restrict.tag.errors.failure', { lng: locale, member: memberMention }));
	}
}
