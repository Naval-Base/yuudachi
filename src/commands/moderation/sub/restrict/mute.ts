...
export async function mute(
	...
	try {
		...
		if (!roles?.mute_role_id) {
			throw new Error(i18next.t('command.mod.restrict.mute.errors.no_role', { lng: locale }));
		}
		...
		const [action] = await sql<[{ action_processed: boolean }?]>`
			...
			where guild_id = ${interaction.guildId}
				...
				and role_id = ${roles.mute_role_id}
			order by created_at desc
			limit 1`;

		if (action && !action.action_processed) {
			throw new Error(
				i18next.t('command.mod.restrict.mute.errors.already_restricted', {
		...
		const roleButton = new MessageButton()
			.setCustomId(roleKey)
			.setLabel(i18next.t('command.mod.restrict.mute.buttons.execute', { lng: locale }))
			.setStyle('DANGER');
		const cancelButton = new MessageButton()
			.setCustomId(cancelKey)
			.setLabel(i18next.t('command.mod.restrict.mute.buttons.cancel', { lng: locale }))
			.setStyle('SECONDARY');

		await interaction.editReply({
			content: i18next.t('command.mod.restrict.mute.pending', {
				user: `${args.user.user.toString()} - ${args.user.user.tag} (${args.user.user.id})`,
				lng: locale,
			}),
			// @ts-expect-error
		...
		if (collectedInteraction?.customId === cancelKey) {
			await collectedInteraction.update({
				content: i18next.t('command.mod.restrict.mute.cancel', {
					user: `${args.user.user.toString()} - ${args.user.user.tag} (${args.user.user.id})`,
					lng: locale,
				}),
				components: [],
			...
			const case_ = await createCase(
				...
				generateCasePayload(collectedInteraction, args, CaseAction.Role, parsedDuration),
			);
			void upsertCaseLog(collectedInteraction, case_);

			await collectedInteraction.editReply({
				content: i18next.t('command.mod.restrict.mute.success', {
					user: `${args.user.user.toString()} - ${args.user.user.tag} (${args.user.user.id})`,
					lng: locale,
				}),
				components: [],
		...
		logger.error(e);
		throw new Error(
			i18next.t('command.mod.restrict.mute.errors.failure', {
				user: `${args.user.user.toString()} - ${args.user.user.tag} (${args.user.user.id})`,
				lng: locale,
			}),
		);
