...
export default class SoftbanAction extends Action {
	...
	public async exec() {
		...
		try {
			try {
				await this.member.send(MESSAGES.ACTIONS.SOFTBAN.MESSAGE(guild.name, this._reason));
			} catch {}
			await this.member.ban({
				days: this.days,
				reason: MESSAGES.ACTIONS.SOFTBAN.AUDIT(this.message.author.tag, totalCases),
			});
			await guild.members.unban(this.member, MESSAGES.ACTIONS.SOFTBAN.AUDIT(this.message.author.tag, totalCases));
		} catch (error) {
