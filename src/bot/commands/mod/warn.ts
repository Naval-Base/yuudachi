...
export default class WarnCommand extends Command {
	...
	public async exec(message: Message, { member, ref, reason }: { member: GuildMember; ref: number; reason: string }) {
		...
		let modMessage;
		if (modLogChannel) {
			const embed = (await Util.logEmbed({ message, db: casesRepo, channel: modLogChannel, member, action: 'Warn', caseNum: totalCases, reason, ref })).setColor(COLORS.WARN);
